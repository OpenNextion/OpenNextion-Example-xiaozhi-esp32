...
#define TAG "OpenNextion_ONX2432G028"
...
public:
    ...
    int Read(int16_t* dest, int samples) {
        ...
        // The data bit width after PDM demodulation is 16 bits, which is read directly into the target buffer.
    ...
    void EnableOutput(bool enable) {
        ...
        // Set NS4168 audio power amplifier shutdown pin, initial state is shutdown to prevent background noise
        if (enable) {
...
class OpenNextion_ONX2432G028 : public WifiBoard {
private:
    ...
    static void touch_event_task(void* arg)
    {
        ...
        while (true) {
            ...
            if (touch_event == CST826::TOUCH_RELEASE) {
                auto &app = Application::GetInstance();
                auto &board = (OpenNextion_ONX2432G028 &)Board::GetInstance();
...
public:
    OpenNextion_ONX2432G028() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeI2c();
        InitializeSpi();
...
DECLARE_BOARD(OpenNextion_ONX2432G028);
